"""
Pessimistic Analyst — 反方辩论

负责:
  拿到报告草稿 → 从谨慎/风险角度审视数据 → 找出潜在问题和隐患

角色类比: 原项目的 Bear Researcher（看空方）
LLM 策略: quick_think_llm，需要鲜明的"谨慎"立场
...
辩论规则:
  - 第一轮: 独立阐述风险点
  - 第二轮+: 逐条反驳 Optimistic 观点，提醒不要盲目乐观
"""
...
PESSIMIST_SYSTEM_PROMPT = load_prompt("pessimist")

PESSIMIST_FIRST_ROUND_PROMPT = (
    "这是第一轮辩论。请独立给出你的风险分析，不要回应乐观方（乐观方还没有发言）。"
)


def _format_debate_history(debate_state: dict, my_side: str = "pessimistic") -> str:
    """格式化完整辩论历史，供双方每轮参考"""
...
def create_pessimist(llm):
    ...
    prompt = ChatPromptTemplate.from_messages(
        [
            ("system", PESSIMIST_SYSTEM_PROMPT),
    ...
    def pessimist_node(state: DataAnalysisState) -> dict:
        """LangGraph 节点函数"""
        debate_state = state.get("debate_state", {})
        round_num = debate_state.get("round_count", 0) + 1
        logger.info("[Pessimistic] 开始辩论 Round %d", round_num)

        draft_report = state.get("draft_report", "")
        optimistic_view = state.get("optimistic_view", "")
        sql_result = state.get("sql_result", "")

        # ─── 构建完整辩论上下文 ───
        full_history = _format_debate_history(debate_state, "pessimistic")

        if round_num <= 1:
            debate_context = (
                "这是第一轮辩论。请独立给出你的风险分析，不要回应乐观方。\n\n"
                + full_history
            )
        else:
            debate_context = (
                f"这是第 {round_num} 轮辩论。\n\n"
                f"对方（乐观方）上一轮的观点:\n{optimistic_view[:2000]}\n\n"
                f"{full_history}\n\n"
                "## 本轮任务（必须逐条完成）:\n"
                "1. **逐一反驳**对方上一轮的核心论点，指出其逻辑漏洞或数据误读\n"
                "2. **引用具体数据**强化自己的立场（优先使用 SQL 结果中的真实数字）\n"
                "3. 如果对方引用了你之前忽略的数据，**必须正面回应**\n"
                "4. 承认对方的合理观点（这展示你的客观性），但说明为什么整体上风险视角更值得关注\n"
        ...
        logger.info("[Pessimistic] Round %d 完成，观点长度: %d", round_num, len(view_content))

        # ─── 更新辩论历史 ───
        debate_state = state.get("debate_state", {})
        prev_history = debate_state.get("pessimistic_history", "")
        new_history = f"{prev_history}\n\n--- Round {round_num} (反方) ---\n{view_content}"

        # ─── 更新共享证据 ───
        shared_evidence = state.get("shared_evidence", {})
        new_data_points = _extract_data_points(view_content, "pessimistic")
        ...
        return {
            "pessimistic_view": view_content,
            "debate_state": {
                **debate_state,
                "pessimistic_history": new_history,
                "latest_speaker": "pessimistic",
                "round_count": round_num,
            },
            "shared_evidence": shared_evidence,
            "messages": [response],
            "progress_message": f"😐 反方辩论 Round {round_num}: 风险视角分析完成",
        }

    return pessimist_node
