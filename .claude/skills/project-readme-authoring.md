...
- 架构图优先用 Mermaid → PNG 静态图（避免依赖 GitHub 渲染）
...
不要只用 Mermaid 代码块（GitHub 可能不渲染）。推荐流程：

1. 写 `.html` 文件，用 Mermaid CDN 渲染
2. Chrome headless 截图生成 PNG
```bash
"C:\Program Files\Google\Chrome\Application\chrome.exe" --headless=new \
  --screenshot="docs/images/diagram.png" --window-size=1100,900 \
  "file:///d:/path/to/diagram.html"
```
3. README 用 `<img>` 引用 PNG：`<img src="docs/images/diagram.png" width="750"/>`
4. 保留 `.html` 和生成脚本以便后续修改
